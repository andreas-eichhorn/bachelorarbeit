...
\begin{document}
    ...
    Das Wasserfallmodell ist ein Modell für Softwareentwicklung \cite{24} und wurde 1970 eingeführt. \cite{25} Die Softwareentwicklung wird dabei in verschiedene voneinander getrennte Phasen unterteilt. \cite{22} 
    ...
    Aus dem Grund ist das Modell besonders für Systeme geeignet, welche nach der Implementierung keine Änderungen mehr zulassen \cite{22}. 
    Die Vorteile eines Wasserfallmodells sind, dass es einfach verständlich ist \cite{22}, die Anforderungen an das System bereits vor der Entwicklung bekannt sind und es einfach zu implementieren ist. \cite{23} 
    Andersseits können sich ändernde Anforderungen oder auftretende Probleme während der Entwicklung nicht berücksichtigt werden. \cite{23} 
    Da das Wasserfallmodell bereits einige Jahre sich bewährt hat, gibt es einige Änderungen am klassichen Wasserfallmodell. 
    So beschreibt TODO (24) in seinem Artikel eine Änderung am Modell so, dass die einzelnen Phasen einer sorgfältigen Validierung und Bestätigung seitens des Clients benötigen um die Phase abzuschließen. 
    Sollte der Client mit dem Ergebnissen nicht zufrieden sein, so kann das Modell wieder von vorne beginnen. \cite{24} 
    In anderen Varianten des Modells wird die Test-Phase komplett weggelassen, da das Testen in Form von Reviews in den frühen Phasen oder durch die Ausführung des Codes in späteren Phasen – während des gesamten Lebenszyklusmodells stattfinden sollte. \cite{25}
    \newpage
    ...
    Bei der Klasse handelt es sich um DataModelActivityElementLoop, die als Grundlage für die Datenspeicherung genutzt wird  und für die Ausführung. TODO
